...
function scrollDown(){
  ...
  chatWindow.scrollTop(chatWindow.prop("scrollHeight") + 1000);
...
function insertToEnd(data, status, xhr){
  //console.log(data);
  newest = data.newest_datetime;
  var list = $('.main').scrollTop() + $('.main').innerHeight() + 50 >= $('.main')[0].scrollHeight;
  var last_block = $('.message_block').last();
  var last_sender = $('.sender').last().text();
  for(var i = 0; i < data.message_blocks.length; i++){
    var message_block = data.message_blocks[i];
    var messages = message_block.messages;
    if(message_block.sender == last_sender){
      for(var j = 0; j < messages.length; j++){
        $('<div>' + messages[j] + '</div>').appendTo(last_block.find('.alert'));
      }
    } else {
      var new_block = getMessageBlock(message_block.sender == usrname, message_block.sender,
        message_block.photo, message_block.messages[0]);
      ...
      new_block.insertAfter(last_block);
      last_block = new_block;
      last_sender = last_block.find('a').first().text();
    }
  }
  if(list && visible)
    scrollDown();
